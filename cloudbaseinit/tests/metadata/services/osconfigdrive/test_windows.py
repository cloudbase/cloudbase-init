...
class TestWindowsConfigDriveManager(unittest.TestCase):
    ...
    @mock.patch("wmi.WMI")
    def test_get_physical_disks_path(self, mock_WMI):
        _mock = mock.MagicMock()
        _mock.DeviceID = 'fake ID'
        mock_WMI().query.return_value = [_mock]
        response = self._config_manager._get_physical_disks_path()
        mock_WMI().query.assert_called_once_with(
            'SELECT DeviceID FROM Win32_DiskDrive')
        self.assertEqual(response[0], 'fake ID')
    ...
    def _test_extract_iso_files(self, mock_makedirs, mock_get_os_utils,
                                exit_code):
        fake_path = os.path.join('fake', 'path')
        ...
        if exit_code:
            self.assertRaises(Exception(),
                              ...
                              fake_path, fake_target_path)
        else:
            self._config_manager._extract_iso_files(fake_path,
                                                    fake_target_path)
    ...
    @mock.patch('cloudbaseinit.metadata.services.osconfigdrive.windows.'
                'WindowsConfigDriveManager._get_physical_disks_path')
    @mock.patch('cloudbaseinit.metadata.services.osconfigdrive.windows.'
    ...
    def _test_extract_iso_disk_file(self, mock_write_iso_file,
                                    mock_PhysicalDisk,
                                    mock_get_iso_disk_size,
                                    mock_get_physical_disks_path, exception):
        fake_path = os.path.join('fake', 'path')
        fake_path_physical = os.path.join(fake_path, 'physical')
        mock_get_iso_disk_size.return_value = 'fake iso size'
        mock_get_physical_disks_path.return_value = [fake_path_physical]

        if exception:
            mock_PhysicalDisk().open.side_effect = [Exception]

        response = self._config_manager._extract_iso_disk_file(fake_path)
        ...
        mock_get_physical_disks_path.assert_called_once_with()
    ...
    def _test_get_conf_drive_from_raw_hdd(self, mock_extract_iso_files,
        ...
        mock_extract_iso_disk_file.assert_called_once_with(fake_iso_path)
        if found_drive:
            mock_extract_iso_files.assert_called_once_with(fake_iso_path,
                                                           fake_target_path)
